...
char* longestPalindrome(char* s) {
    int n = strlen(s);    // 取得輸入字串 s 的長度

    // 初始化一個 n x n 的二維布林陣列 dp
    // dp[i][j] 將用於記錄從索引 i 到 j 的子字串是否為回文
    bool dp[n][n];

    // 使用 memset 函式將整個 dp 陣列初始化為 false0
    ...
    // 在最長回文子字串的結束索引之後加上一個空字元 \0
    // 這將字串 s 截斷，使其只包含最長回文的部分
    s[ans[1] + 1] = '\0';

    // 回傳一個指向最長回文子字串起始位置的指標
    // 因為 C 語言中的字串是連續的記憶體區塊，所以這種操作是可行的
    return s + ans[0];
}
